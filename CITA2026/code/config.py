...
DATASET_REGISTRY: dict[str, DatasetConfig] = {
    ...
    "kitchenham": DatasetConfig(
        name="kitchenham",
        path=_CLEANED / "kitchenham_processed.csv",
    ...
    "miyazaki94": DatasetConfig(
        name="miyazaki94",
        path=_CLEANED / "miyazaki94_processed.csv",
        target_column="MM",
        drop_columns=(),
    ),
}
...
class ExperimentConfig:
    ...
    pso_convergence_plot_dir: Path | None = None
    """If set, save PSO convergence curve (fitness vs iteration) per model to this directory."""

    # GWO hyperparameter optimization
    use_gwo: bool = False
    """Whether to use GWO for hyperparameter optimization."""
    gwo_n_wolves: int = 30
    """Number of wolves in the GWO pack (population size)."""
    gwo_n_iterations: int = 50
    """Number of GWO hunting iterations (epochs)."""
    gwo_cv_folds: int = 5
    """Number of CV folds for GWO fitness evaluation."""
    gwo_convergence_plot_dir: Path | None = None
    """If set, save GWO convergence curve (alpha fitness vs iteration) per model to this directory."""
