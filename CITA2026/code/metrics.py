...
def compute_mmre(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """
    Mean Magnitude of Relative Error:
        MRE_i = |y_i - y_hat_i| / y_i  (only for y_i > 0)
        MMRE = mean(MRE_i) over valid samples.
    """
    y_true = np.asarray(y_true, dtype=np.float64).ravel()
    y_pred = np.asarray(y_pred, dtype=np.float64).ravel()
    valid = y_true > 0
    if not np.any(valid):
        return float("nan")
    mre = np.abs(y_true[valid] - y_pred[valid]) / y_true[valid]
    return float(np.mean(mre))


def compute_pred25(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """
    Pred(25): percentage of predictions within 25% of actual.
        MRE_i = |y_i - y_hat_i| / y_i; count where MRE_i <= 0.25, then Pred25 = 100 * count / n.
    Uses only samples with y_i > 0 for MRE; denominator is full n.
    """
    y_true = np.asarray(y_true, dtype=np.float64).ravel()
    y_pred = np.asarray(y_pred, dtype=np.float64).ravel()
    n = len(y_true)
    ...
    if not np.any(valid):
        return 0.0
    mre = np.abs(y_true[valid] - y_pred[valid]) / y_true[valid]
    within_25 = np.sum(mre <= 0.25)
    return 100.0 * within_25 / n


def compute_all_regression_metrics(
    y_true: np.ndarray, y_pred: np.ndarray
) -> dict[str, float]:
    """Return dict with MAR, MSE, RMSE, MAE, R², MMRE, Pred25 for y_true vs y_pred."""
    y_true = np.asarray(y_true, dtype=np.float64).ravel()
    y_pred = np.asarray(y_pred, dtype=np.float64).ravel()
    return {
        ...
        "MMRE": compute_mmre(y_true, y_pred),
        "Pred25": compute_pred25(y_true, y_pred),
    }
